...
} from './interfaces/admins.repository.interface';
...
export class AdminsService implements IAdminsService {
  ...
  constructor(
    ...
    private readonly _reportsService: IReportService,
  ...
  async approveRequest(
    ...
    try {
      const success = await this._editorService.approveEditorRequest(
        requestId,
        adminId,
      );
  ...
  async rejectRequest(
    ...
    try {
      const success = await this._editorService.rejectEditorRequest(
        requestId,
        reason,
      );
      return success ? true : false;
