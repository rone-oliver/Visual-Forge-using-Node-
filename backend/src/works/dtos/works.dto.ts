...
export class UpdateWorkFilesDto {
  @ApiPropertyOptional({
    description: 'An array of unique IDs of the files to be deleted.',
    type: [String],
  })
  @IsOptional()
  @IsArray()
...
export class GetPublicWorksQueryDto {
  ...
  @ApiPropertyOptional({
    description: 'Search term for filtering public works',
  })
  @IsString()
  @IsOptional()
  search?: string;
...
export class PublicWorkItemDto {
  ...
  @ApiProperty({
    ...
    description: 'Final files delivered for the work',
  })
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => FileUploadResultDto)
