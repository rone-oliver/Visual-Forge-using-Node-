...
export class NotificationService implements INotificationService {
  ...
  constructor(
    ...
    private readonly _notificationGateway: INotificationGateway,
  ) {}
  ...
  async createNotification(
    ...
    try {
      ...
      try {
        ...
        this._logger.error(
          `Failed to send notification via WebSocket: ${socketError.message}`,
          socketError.stack,
        );
  ...
  async handleBidAcceptedEvent(payload: {
    ...
    try {
      ...
      this._logger.error(
        `Failed to handle bid accepted event: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleQuotationCreatedEvent(payload: {
    ...
    this._logger.log(
      `Handling quotation created event for quotation ${payload.quotationId}`,
    );
    ...
    try {
      ...
      this._logger.error(
        `Failed to handle quotation created event: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleQuotationCompletedEvent(payload: {
    ...
    this._logger.log(
      `Handling quotation completed event for quotation ${payload.quotationId}`,
    );
    ...
    try {
      ...
      await this.createNotification({
        userId: payload.userId.toString(),
        message:
          payload.message || 'You have successfully completed a quotation',
      ...
      this._logger.error(
        `Failed to handle quotation completed event: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handlePaymentRefunded(payload: {
    ...
    this._logger.log(
      `Handling payment refunded event for user ${payload.recipient}`,
    );
    await this.createNotification({
  ...
  async handleEditorWarning(payload: {
    recipient: Types.ObjectId;
    message: string;
    type: NotificationType;
  }) {
    this._logger.log(
      `Handling editor warning event for editor ${payload.recipient}`,
    );
    await this.createNotification({
      userId: payload.recipient.toString(),
      message: payload.message,
  ...
  async handleEditorSuspended(payload: {
    recipient: Types.ObjectId;
    message: string;
    type: NotificationType;
  }) {
    this._logger.log(
      `Handling editor suspended event for editor ${payload.recipient}`,
    );
    await this.createNotification({
      userId: payload.recipient.toString(),
      message: payload.message,
  ...
  async markAllAsRead(userId: string): Promise<any> {
    const result = await this._notificationRepository.updateMany(
      { userId, unread: true },
      { unread: false },
    );
  ...
  async deleteNotification(notificationId: string): Promise<boolean> {
    try {
      ...
      try {
        ...
        this._logger.error(
          `Failed to send deletion update via WebSocket: ${socketError.message}`,
          socketError.stack,
        );
      }
