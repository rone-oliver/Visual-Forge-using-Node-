...
export class NotificationRepository implements INotificationRepository {
    constructor(
        @InjectModel(Notification.name) private readonly _notificationModel: Model<NotificationDocument>,
    ) { }
    ...
    async findByUserId(userId: string): Promise<Notification[]> {
        return this._notificationModel.find({ userId: new Types.ObjectId(userId) }).sort({ createdAt: -1 }).exec();
    }

    async findUnreadByUserId(userId: string): Promise<Notification[]> {
        return this._notificationModel.find({ userId: new Types.ObjectId(userId), unread: true }).sort({ createdAt: -1 }).exec();
    }
    ...
    async update(id: string, data: Partial<Notification>): Promise<Notification | null> {
        return this._notificationModel.findByIdAndUpdate(id, data, { new: true }).exec();
    }

    async updateMany(filter: {userId: string, unread: boolean}, data: Partial<Notification>): Promise<any> {
        return this._notificationModel.updateMany({ userId: new Types.ObjectId(filter.userId), unread: filter.unread }, data).exec();
    }
    ...
    async countUnread(userId: string): Promise<number> {
        return this._notificationModel.countDocuments({ userId: new Types.ObjectId(userId), unread: true }).exec();
    }
}