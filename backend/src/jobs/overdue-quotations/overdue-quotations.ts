...
export class OverdueQuotationsService {
    ...
    async handleOverdueQuotations() {
        ...
        this._logger.log(`Found ${overdueQuotations.length} overdue quotations to process.`);
        ...
        const processingPromises = overdueQuotations.map(async (quotation) => {
            try {
                ...
                if (typeof quotation.advanceAmount === 'number' && quotation.advanceAmount > 0) {
                    ...
                    this._logger.warn(`Skipping refund for quotation #${quotation._id} due to missing or invalid advanceAmount.`);
                }

                // 2. Handle editor warning and suspension
                const editor = await this._editorService.findByUserId(quotation.editorId);
                if (editor) {
                    ...
                    if(isSuspended){
                        ...
                        if(editor.suspendedUntil && editorUser){
                            await this._mailService.sendSuspensionEmail(editorUser.email, { suspendedUntil: editor.suspendedUntil });
                        }
                    } else {
                        ...
                        if(editorUser){
                            await this._mailService.sendWarningEmail(editorUser.email, { quotationTitle: quotation.title });
                        }
                    }
                }

                await this._quotationService.updateQuotation({ _id: quotation._id }, { status: QuotationStatus.EXPIRED });

                this._logger.log(`Successfully processed overdue quotation #${quotation._id.toString()}`);

            } catch (error) {
                this._logger.error(`Failed to process overdue quotation #${quotation._id.toString()}: ${error.message}`, error.stack);
            }
        })
    ...
    async handleWarningReset() {
        ...
        try {
            ...
            this._logger.log(`Found ${editorsToReset.length} editors for warning reset.`);

            for (const editor of editorsToReset) {
                await this._editorService.updateEditor(editor.userId, { warningCount: 0 });
                this._logger.log(`Reset warning count for editor #${editor.userId}`);
            }

        } catch (error) {
            this._logger.error(`Failed to run warning reset job: ${error.message}`, error.stack);
        }
