...
export class AiService implements IAiService {
  ...
  async generateSmartReplies(
    ...
    if (!lastMessage || lastMessage.role !== 'model') {
      this._logger.log(
        `Skipping smart replies: Last message was from role '${lastMessage?.role || 'unknown'}', not 'model'.`,
      );
      return [];
    }
    ...
    if (firstUserIndex === -1) {
      this._logger.log(
        'Skipping smart replies: No user message found in history.',
      );
      return [];
    }
      ...
      const result = await model.generateContent({
        contents: [
          ...validHistory,
          {
            ...
            parts: [
              {
                text: 'Based on the last message from the "model", provide three concise reply suggestions for the "user".',
              },
            ],
          },
        ],
