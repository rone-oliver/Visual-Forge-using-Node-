...
export class EditorRepository implements IEditorRepository {
  constructor(
    ...
    private readonly _editorModel: Model<EditorDocument>,
  ) {}
  ...
  async updateScore(
    ...
    await this._editorModel.updateOne(
      { _id: editorId },
      { $set: { score, streak } },
    );
  }

  async addSharedTutorial(
    userId: string,
    tutorialUrl: string,
  ): Promise<Editor> {
    const updatedEditor = await this._editorModel
      .findOneAndUpdate(
