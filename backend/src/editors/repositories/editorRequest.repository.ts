...
export class EditorRequestsRepository implements IEditorRequestsRepository {
  constructor(
    ...
    private readonly _editorRequestModel: Model<EditorRequestDocument>,
  ) {}

  async countEditorRequests(): Promise<number> {
    return this._editorRequestModel.countDocuments({
      status: EditorRequestStatus.PENDING,
    });
  }

  async getEditorRequests(): Promise<EditorRequest[]> {
    return this._editorRequestModel
      .find({ status: EditorRequestStatus.PENDING })
      .populate('userId');
  }
  ...
  async approveEditorRequest(
    ...
    return this._editorRequestModel.findByIdAndUpdate(
      requestId,
      {
  ...
  async deleteRequest(userId: Types.ObjectId): Promise<EditorRequest | null> {
    return this._editorRequestModel.findOneAndDelete({
      userId: new Types.ObjectId(userId),
    });
  }
}
