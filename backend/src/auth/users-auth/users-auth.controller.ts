...
import {
  IUsersAuthService,
  IUsersAuthServiceToken,
} from './interfaces/usersAuth-service.interface';

@Controller('auth/user')
export class UsersAuthController {
  constructor(
    @Inject(IUsersAuthServiceToken)
    private readonly _usersAuthService: IUsersAuthService,
  ) {}
  ...
  async resetPassword(@Body() body: { email: string; newPassword: string }) {
    const result = await this._usersAuthService.resetPassword(
      body.email,
      body.newPassword,
    );
    return result;
