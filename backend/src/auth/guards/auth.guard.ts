  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (contextType === 'ws') {
      ...
      try {
        ...
        if (!authToken) {
            this.logger.warn(`WS Auth: No token provided for client ${client.id}. Disconnecting.`);
            client.disconnect();
            return false;
        }
        ...
        this.logger.log(`WS Auth: Client ${client.id} authenticated as user ${payload.userId}`);
        console.log('AuthGuard canActivate ws:', context.getType())
        return true;
      } catch (error) {
