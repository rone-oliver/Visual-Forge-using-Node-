...
export class PaymentService implements IPaymentService {
  ...
  async createRazorpayOrder(
    amount: number,
    currency: string = this._DEFAULT_CURRENCY,
    quotationId: string,
    receipt?: string,
  ) {
    try {
  ...
  async reconcileStuckPayments(quotationService: IQuotationService) {
    try {
      ...
      if (!stuckQuotations) {
        this._logger.warn(
          'No quotations found for clearing the payment states',
        );
        return;
      }
  ...
  async handleWebhook(
    event: string,
    data: any,
    quotationService: IQuotationService,
  ) {
    try {
      switch (event) {
        case 'payment.failed':
        case 'order.expired':
          if (data.order_id) {
            const quotation = await quotationService.findOneByRazorpayOrderId(
              data.order_id,
            );
            if (quotation) {
              await quotationService.updateQuotation(quotation._id, {
                isPaymentInProgress: false,
                advancePaymentOrderId:
                  data.order_id === quotation.advancePaymentOrderId
                    ? null
                    : quotation.advancePaymentOrderId,
                balancePaymentOrderId:
                  data.order_id === quotation.balancePaymentOrderId
                    ? null
                    : quotation.balancePaymentOrderId,
              });
            }
          }
          break;
  ...
  async verifyPayment(
    ...
    if (generatedSignature !== razorpaySignature) {
      this._logger.warn(
        `Payment verification failed for orderId: ${razorpayOrderId}`,
      );
    ...
    try {
      ...
      this._logger.error(
        `Failed to fetch payment details for paymentId: ${razorpayPaymentId}`,
        error,
      );
  ...
  async getAccountBalance(): Promise<number> {
    try {
      ...
      const authToken = Buffer.from(
        `${this._razorpayKeyId}:${this._razorpayKeySecret}`,
      ).toString('base64');
      ...
      if (data && data.items && data.items.length > 0) {
        const currentAccount = data.items.find(
          (item: any) =>
            item.account_type === RazorpayAccountType.RAZORPAYX_LITE,
        );
      ...
      this._logger.error(
        'Error fetching Razorpay banking balance:',
