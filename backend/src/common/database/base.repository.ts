...
import { Document, FilterQuery, InsertManyOptions, Model, ProjectionType, Types, UpdateQuery } from "mongoose";
...
export class BaseRepository<T, D extends Document> implements IBaseRepository<T, D>{
    ...
    async findById(id: string | Types.ObjectId, projection?: ProjectionType<T>): Promise<D | null> {
        return this.model.findById(id, projection).exec();
    }

    async findOneAndUpdate(filter: FilterQuery<T>, update: UpdateQuery<T>): Promise<D | null> {
        return this.model.findOneAndUpdate(filter, update as any, { new: true }).exec();
    }
