...
export class RelationshipService implements IRelationshipService {
  ...
  async createRelationship(dto: RelationshipDto): Promise<Relationship> {
    ...
    try {
      ...
      if (error.code === 11000) {
        throw new ConflictException(
          `Relationship '${type}' already exists between these users.`,
        );
      }
      throw error;
    }
  ...
  async removeRelationship(
    ...
    if (result.deletedCount === 0) {
      throw new NotFoundException(
        `Relationship '${dto.type}' not found between these users.`,
      );
    }
    return result;
  }
  ...
  async getFollowing({
    ...
    limit,
    skip,
  }: GetRelatedUsersDto): Promise<UserDocument[]> {
