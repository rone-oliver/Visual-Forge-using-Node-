...
export class CloudinaryService implements ICloudinaryService {
        ...
        const uploadStream = cloudinary.uploader.upload_stream(
          ...
          (error, result) => {
            ...
            if (result) {
              ...
              this._logger.error(
                'Cloudinary upload successful, but result is undefined.',
              );
              return reject(
                new Error(
                  'Cloudinary upload successful, but result is undefined.',
  ...
  async deleteFile(
    ...
    try {
      ...
      this._logger.error(
        `Error deleting file ${publicId} from Cloudinary: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private _getResourceType(
    fileType: FileType,
  ): UploadApiResponse['resource_type'] {
    switch (fileType) {
      case FileType.IMAGE:
        return 'image';
      case FileType.VIDEO:
    ...
    switch (fileType) {
      ...
      case FileType.AUDIO:
        return {
          ...
          eager: [{ format: 'mp3', transformation: [{ quality: 'auto' }] }],
          eager_async: true,
          access_mode: 'public',
